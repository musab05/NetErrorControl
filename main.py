...
# Drawing functions
def draw_grid(grid, start, end, player=None):
    SCREEN.fill(WHITE)
...
def main():
    ...
    while running:
        ...
        if path and step_index < len(path):
            ...
            path = []  # Clear path when finished

        # Only calculate time_elapsed while pathfinding is in progress
        if path and step_index < len(path):
            time_elapsed = time.time() - start_time
        else:
            time_elapsed = time.time() - start_time if step_index > 0 else 0  # Freeze time when path is finished

        draw_grid(grid, start, end, player)
